...
const App = () => {
  ...
  function nextRound (initialWord: string | null) {
    console.log("initialWord:", initialWord);
    console.log("wordBank:", wordBank);
    if (initialWord) {
      setAnswer(initialWord);
      setWordBank((prevWordBank) => prevWordBank.filter((word) => word !== initialWord));
      console.log("Initial Word Answer:", initialWord); // For debugging
    } else if (wordBank.length > 0) {
      const nxtAnswer = wordBank[Math.floor(Math.random() * wordBank.length)]
      setAnswer(nxtAnswer);
      setWordBank((prevWordBank) => prevWordBank.filter((word) => word !== nxtAnswer));
      console.log("nxtWord Answer:", nxtAnswer); // For debugging
    } else {
      alert("WOW! You crashed all 30 words! Let's get some more words for ya!");
  ...
  const handleSubmit = () => {
    ...
    if (guess === answer) {
      ...
      setGameOver(true);
      alert(`Game Over! The word was ${answer}. You wanna start a new game?`);
  ...
  return (
    <div className="flex flex-col items-center p-4 min-h-screen bg-gray-100">
      ...
      {gameOver && (
        <>
        <div className="mb-4">
          <p>Game Over! Start a new game?</p>
        </div>
        <button
